...
```{r Simulate again this time with allels}
set.seed(1)
...
```{r}
data <- read.delim("data/schunkert_risk_allels.csv", sep = " ", header = FALSE)
```


```{r}
df <- read_delim("data/schunkert_risk_allels_cleaned.csv", col_names = c("SNP", "CHR", "BP", "A1", "OR", "P_VALUE", "Risk_allele_frequency_UKB"), delim = " ") %>% 
  mutate(
    CHR = as.numeric(CHR),
    BP = as.numeric(BP),
    OR = as.numeric(OR),
...
ggplot(df, aes(x = Risk_allele_frequency_UKB, y = OR)) +
  geom_point() +
  labs(
